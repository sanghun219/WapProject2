...
{   //4x3 행렬으로 맵을만듬(y,x)형태
    ...
    public bool isClearAllDungeon = false;
    public int NumOfClearDungeon = 0;
    ...
    //StartXpos,StartYpos는 시작 포지션의 의미
    //이 포지션을 계속이용해 현재 플레이어가 있는 방의 위치를 확인함.
    ...
    public void UpdatePlayerPosinMinimap()
    {
        ...
        {
            if (!playerTransform.Find("Player")) return;
            playerTransform.Find("Player").GetComponent<RectTransform>().localPosition =
          new Vector3(-100 + rooms[StartYpos, StartXpos].xPos * GameObject.Find("MiniMap").transform.Find("Player").GetComponent<RectTransform>().rect.width / 1.3f
        ...
        for (int y = 0; y < rooms.GetLength(0); y++)
            for (int x = 0; x < rooms.GetLength(1); x++)
            {
                ...
                if (rooms[y, x].roomType == Room.ROOMTYPE.BOSS)
                {
                    ...
                    newImage.rectTransform.localPosition = new Vector3(-100 + rooms[y, x].xPos * newImage.rectTransform.rect.width / 1.3f, 60 - 1 * rooms[y, x].yPos
                ...
                if (rooms[y,x].yPos == StartYpos && rooms[y,x].xPos == StartXpos)
                {
                    ...
                    newImage.gameObject.SetActive(false);
                    newImage.rectTransform.localPosition = new Vector3(-100 + rooms[y, x].xPos * newImage.rectTransform.rect.width / 1.3f, 57 - 1 * rooms[y, x].yPos
                        * newImage.rectTransform.rect.height / 1.3f, 1.0f);
    ...
    //모든 방 클리어시 작동
    public void FindClearRoom()
    {
        if (isClearAllDungeon) return;
        for (int i = 0; i < rooms.GetLength(0); i++)
            for (int j = 0; j < rooms.GetLength(1); j++)
            {
                if (rooms[i, j].roomType != Room.ROOMTYPE.BLOCK && rooms[i, j].IsEmptyMonster)
                {
                    NumOfClearDungeon++;
                }
        ...
        if (NumOfClearDungeon == maxRoomNums)
        {
            isClearAllDungeon = true;
